...
// runs locally and produces a snapshot metadata record alongside the
// normal state record.
//
...
func BuildCapture(req operation.Request, cfg *config.Config, source *environment.Resolved, snapshotID string) (*Plan, error) {
	...
	for _, res := range req.Resources {
		...
		if !ok {
			available := sortedKeys(sourceEnv.Capture)
			if len(available) == 0 {
...
func BuildRestore(req operation.Request, cfg *config.Config, target *environment.Resolved, snapshotID, snapshotEnv string) (*Plan, error) {
	...
	for _, res := range req.Resources {
		...
		if !ok {
			available := sortedKeys(targetEnv.Restore)
			if len(available) == 0 {
...
// sortedKeys returns the keys of m in sorted order. Used only to
